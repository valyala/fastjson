...
// NewNumberUint64 returns new number value containing n.
//
// The returned number is valid until Reset is called on a.
func (a *Arena) NewNumberUint64(n uint64) *Value {
	v := a.c.getValue()
	v.t = TypeNumber
	bLen := len(a.b)
	a.b = strconv.AppendUint(a.b, n, 10)
	v.s = b2s(a.b[bLen:])
	return v
}

// NewNumberUint32 returns new number value containing n.
//
// The returned number is valid until Reset is called on a.
func (a *Arena) NewNumberUint32(n uint32) *Value {
	v := a.c.getValue()
	v.t = TypeNumber
	bLen := len(a.b)
	a.b = strconv.AppendUint(a.b, uint64(n), 10)
	v.s = b2s(a.b[bLen:])
	return v
}
